...
- [各プロセスの手法と操作手順](#各プロセスの手法と操作手順)
  ...
  - [5. PTEE(-R²) スペクトルフィッティング](#5-pteer-スペクトルフィッティング)
  - [6. PCA + clustering / SVD（多変量解析）](#6-pca--clustering--svd多変量解析)
...
スキャンを開く
   └─▶ スペクトル ROI 選択・エネルギー移動（メインウィンドウ）
        └─▶ [OD] 直接ビーム ROI から OD を算出
             ...
             │        └─▶ アプライすると同時にプリマップも自動作成
             ├─▶ [Pre-map] 正味吸収マップ／ピークマップ（範囲を変えるときだけ開く）
             │        └─▶ [Segmentation] 閾値クラスタリング＋クラスタ平均スペクトル
             │                 └─▶ [PTEE(-R²)] 参照スペクトルとの R² マッピング
             └─▶ [PCA + clustering / SVD] 多変量分解とマップ
```
...
- **レイヤーリスト**: 中央画像に表示する対象を切り替えます。基本の
  ...
  `OD - pre-edge (OD)`、`PTEE RGBY map`、`PCA cluster map` など）が追加されます。
- **フレーム移動**: スライダーは廃止しました。右側の **Frame** ボックスに
  フレーム番号を入力して Enter で移動するか、**スペクトルプロット上を
  クリック・ドラッグ**して黄色いエネルギーカーソルをその位置へ動かします
  （最も近いフレームが選択され、ボックスとエネルギーラベルが更新されます）。
  ボックス隣のラベルに現在フレームのエネルギーが表示されます。
- **スペクトルプロファイル**: **ROI 未選択のときは全画素の平均スペクトル**
  （透過または OD）を灰色の破線で表示します。スペクトル ROI を 1 つ以上置くと
  全画素平均は消え、各 ROI の平均スペクトルに置き換わります。この挙動は
  出力レポートにも反映されます。
- **スペクトル ROI**: `ROI ▸ Select spectrum ROI` で矩形 ROI を置くと、その領域の
  平均スペクトルがプロットされます。ROI は複数置けます。
...
4. 下の**画像ビューで結果を確認**します。スライダーで補正後フレームを順に見て、
   **「Show original (compare)」**にチェックすると入力（補正前）と補正後を
   見比べられます。
5. 問題なければ「Apply registered stack」を押すとメインウィンドウに反映され、
   レイヤーリストに `Registered OD`（または `Registered transmission`）が追加され、
   **さらに補正後スタックから既定範囲でプリマップが自動作成されます**（次節参照）。
...
- **OD − pre-edge スタック**（`pre_edge_subtract`）: 各フレームからプリエッジ平均を
  ...
  `OD - pre-edge (…)` として追加され、エネルギーごとに確認できます。
...
- 任意で **SVD/PCA 低ランク再構成による denoising** を前処理として適用できます
  （下位主成分を捨ててノイズを低減。[PCA + clustering / SVD](#6-pca--clustering--svd多変量解析) と同じ分解を使用）。

**作成のタイミング**: レジストレーションをアプライすると、既定範囲
（プリエッジ＝スキャン先頭 1/5、ポストエッジ＝残り）でプリマップが自動作成されます。
**範囲を変えたい（または denoising を有効にしたい）ときだけ Pre-map ウィンドウを開き**、
ボタンを押して再作成します。
...
選択できます。なお、クラスタの平均スペクトル（プロファイル）はどの入力を選んでも
常に**生 OD**から算出されます（オリジナルと同一）。

**閾値の設定**: 上下の閾値スライダーは廃止しました。閾値は、**ヒストグラム上を
ドラッグ**する（カーソルに**近い側**の閾値がそこへ移動）か、「Log Y」チェックボックスの
隣にある **Lower / Upper** の 2 つのボックスに数値を入力して設定します。ドラッグすると
ボックスの値が、ボックスに入力するとヒストグラム上の範囲が更新され、赤いオーバーレイと
選択面積の表示もリアルタイムに追従します。
...
2. ヒストグラムをドラッグするか Lower / Upper ボックスに入力して閾値を調整
   （赤いオーバーレイと面積表示で確認）。
...
### 5. PTEE(-R²) スペクトルフィッティング

**手法**

ピーク標的エンドメンバー抽出（PTEE）と R² RGBY マッピング。各画素の OD スペクトルを
参照スペクトル（セグメンテーションで保存した平均プロファイル等）と比較し、一致度を
決定係数 R² で評価してカラーマップ化します
（`muaxis.processing.fitting.spectral_r2_map`）。

- 参照は最大 4 本を R / G / B / Y チャンネルに割り当て。
- 画素ごとに残差平方和と全変動から標準的な決定係数
  `R² = 1 − Σ(y−r)² / Σ(y−ȳ)²` を計算（固定参照に対する R² であり負値も取り得ます。
  切片付き回帰の Pearson 相関の二乗ではありません）。
- **分類器（Classifier、コンボ）**: 各画素を参照とどう照合するかの規則。
  `R²`（既定・上記、最軽量で解釈が容易）、`SAM`（スペクトル角のコサイン類似度）、
  `LCF`（非負線形結合の分率存在量、高速な OLS＋クリップ近似）から選べます。SAM/LCF は
  微小相の過検出を抑えます。LCF はスコアが分率存在量なので、使用時は score floor を下げて
  ください。R²/SAM/LCF は floor・単相割り当て・RGBY 表示ロジックを共有します（スコアが高い
  ほど良一致）。
- **score floor** を下限として `[floor, 1]` を `[0, 1]` にクリップした重みを各チャンネルへ。
  Y チャンネルは R・G に加算されて表示色（RGBA、プリマルチプライ済み）になります。
- **単相割り当て（Single-phase assignment、チェックボックス）**: 既定では 1 画素が
  複数チャンネルの混合（mixture）として表示されます。これを有効にすると、各画素を
  **最もスコアの高い参照 1 本**に割り当て（winner-take-all）、1 画素 1 相のマップになります。
  最良一致が floor を下回る画素は未割り当てのままです。
...
2. `Process ▸ PTEE(-R²)` を開く。
3. R / G / B / Y の各チャンネルに、保存済みラベルとその中のプロファイル（クラスタ）を割り当て
   （不要なチャンネルは「None」）。選択した参照スペクトルは上部にプレビュー表示されます。
4. 分類器（R² / SAM / LCF）、正規化方式、score floor、必要なら単相割り当て・ガウシアンぼかし（半径）を設定。
5. 「Run」でマッピングを実行。下部に RGBY マップが表示されます。
6. 「Save map」を押すとメインウィンドウのレイヤーリストに `PTEE RGBY map` が
   追加され、中央画像に表示できます。

---

### 6. PCA + clustering / SVD（多変量解析）
...
- **PCA + k-means クラスターマップ**も作成できます。

**操作手順**

1. `Process ▸ PCA + clustering` を開く（プリマップの denoise 済みスタックがあればそれを、
   なければ OD／透過スタックを入力）。
...
4. 「Run」を押すと別スレッドで分解が走り、進捗バーが進みます。完了後、上部に
   成分スペクトル、下部にマップが表示されます（レイヤーリストの
   `SVD/PCA RGB map` または `PCA cluster map`）。
...
│   ├── fitting.py         # 参照スペクトルとの R² RGBY マッピング（PTEE）
